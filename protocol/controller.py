...
# class QncController(Controller):

#     def _route_computation(self):
#         database = {}
#         with open("./experiments/network/qnc/db.json") as json_file:
#             db = json.load(json_file)
#             database["qrx"] = {k: QuantumRouter.parse_raw(v) for k, v in db["qrx"].items()}
#             database["qrp"] = {k: QuantumRepeater.parse_raw(v) for k, v in db["qrp"].items()}
#             database["qhs"] = {k: HeraldingStation.parse_raw(v) for k, v in db["qhs"].items()}
#             database["quantlink"] = {k: QuantLink.parse_raw(v) for k, v in db["quantlink"].items()}

#         for link in database["quantlink"].values():
#             node_1 = database["qhs"].get(
#                 link.node_1, database["qrp"].get(
#                     link.node_1, database["qrx"].get(
#                         link.node_1, None
#                     )
#                 )
#             )
#             node_2 = database["qhs"].get(
#                 link.node_2, database["qrp"].get(
#                     link.node_2, database["qrx"].get(
#                         link.node_2, None
#                     )
#                 )
#             )
...
#             if node_1.interfaces is None:
#                 node_1.interfaces = []
#             if node_2.interfaces is None:
#                 node_2.interfaces = []

#             node_1.interfaces.append(link.interface_1)
#             node_2.interfaces.append(link.interface_2)

#         self._routing: Routing = Routing()
#         qnc_experiment_topology(self._routing, database)
#         self._routing.compute_routes()
